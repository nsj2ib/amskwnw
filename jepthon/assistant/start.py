...
async def start(event):
    ...
    if event.sender_id == bot.uid:
        ...
        await tgbot.send_message(
            ...
            link_preview=False,
            buttons=[
                [custom.Button.inline("تنـصيب 𝑠𝑜𝑢𝑟𝑠𝑒 𝑠𝑜𝑢𝑛𝑑𝑤𝑎𝑣𝑒  🐍", data="deploy")],
                [Button.url("تحتاج مسـاعدة ❓", "https://t.me/L_URD")],
            ],
        )
...
async def help(event):
    ...
    if event.query.user_id is not bot.uid:
        await tgbot.send_message(
            ...
            buttons=[
                [Button.url("شرح التنصيب 📺", "https://youtu.be/WWEEHHHH")],
                [Button.url("كروب المساعدة ❓", "https://t.me/+NOsvthOdPCdkODli")],
            ],
        )
...
async def settings(event):
    if event.sender_id == bot.uid:
        ...
        await event.answer("انت لا تستطيع استخدام البوت احصل على بوتك من @WWEEHHHH", alert=True)




@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"rozname"))) 
async def settings(event):  #    قـسـم  الزغرفـة 𝑠𝑜𝑢𝑟𝑠𝑒 𝑠𝑜𝑢𝑛𝑑𝑤𝑎𝑣𝑒
    if event.sender_id == bot.uid:
        await event.delete()
        await tgbot.send_message(event.chat_id,
